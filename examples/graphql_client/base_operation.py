...
class GraphQLArgument:
    ...
    def _convert_value(
            self, value: Any
    ) -> Union[StringValueNode, IntValueNode, FloatValueNode, BooleanValueNode, ObjectValueNode, ListValueNode]:
        ...
        if isinstance(value, list):
            return ListValueNode(values=[self._convert_value(v) for v in value])
        if isinstance(value, BaseModel):
            fields = [
                ObjectFieldNode(name=NameNode(value=field_info.alias or field_name),
                                value=self._convert_value(field_value))
